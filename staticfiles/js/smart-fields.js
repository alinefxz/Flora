document.addEventListener("DOMContentLoaded", () => {
    ...
    async function readJson(response) {
      ...
      throw new Error(
        "O servidor encontrou um erro. Tente novamente."
      );
    }
  
    async function requestJson(url, options = {}) {
      // CORREÇÃO CRÍTICA: Evita o erro 405 / HTML parse error do Django em requisições POST
      let finalUrl = url;
      if (options.method && options.method.toUpperCase() === "POST" && !finalUrl.endsWith("/")) {
          finalUrl += "/";
      }
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        try {
          const url = (
            `${form.dataset.verifyUrl}?codigo_barras=${code}`
          );
